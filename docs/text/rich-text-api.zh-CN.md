...
笔的操作：`color(int|TextColor|ChatFormatting|StyleVar<Integer>)`、`bold()`、`italic()`、
...
笔的颜色可以引用主题槽位而不是字面量：`color(StyleVar<Integer>)` 接受任意 `StyleVar` 颜色透镜，槽位在**渲染时**每帧对当前主题解析一次。文档始终是纯数据——切换主题层或热重载主题，下一帧就重新上色，无需重建任何节点。

```java
public static final StyleVar<Integer> warning = StyleVar.color("--warning");

RichText.builder()
        .text("按 ").color(0xFFAA00).color(warning)   // 主题槽位，橙色作为回退
        .text(" 继续")
        .build();
```
...
1. 当前主题解析出该槽位——以该值为准，包含 alpha（槽位绑定为 `transparent` 时不画任何东西）；
2. 当前主题**未**解析出该槽位——回退到笔上先前设置的字面量颜色；若没有则透明，因此文档在请求主题色时不会悄悄用渲染器默认色；
3. 未绑定槽位——字面量颜色，否则渲染器默认色。

widget 通过自身的 style context 解析（`ThemeColorResolver.of(style())`），因此内联 `--x` 写入与场景级主题覆盖都生效；tooltip 这类脱离 widget 树的渲染解析当前主题的 `:root` 槽位（`ThemeColorResolver.ofActiveTheme()`）。完整链路见 `RichTextStyle#textColor(ThemeColorResolver, int)`。
