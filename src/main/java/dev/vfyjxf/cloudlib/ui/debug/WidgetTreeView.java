...
final class WidgetTreeView extends WidgetGroup<Widget> {
    ...
    private void scrollToSelected() {
        ...
        if (index < 0) return;
        float rowTop = index * (float) DebugTheme.rowHeight;
        float viewTop = scrollState.scrollY();
        float viewBottom = viewTop + height();
        if (rowTop < viewTop || rowTop + DebugTheme.rowHeight > viewBottom) {
    ...
    private final class TreeRow extends Widget {
        ...
        TreeRow(Widget node, int depth, boolean hasChildren, boolean isExpanded) {
            ...
            onMouseClicked((input, context) -> {
                ...
                int arrowLeft = depth * DebugTheme.rowIndent;
                if (hasChildren && localX >= arrowLeft && localX <= arrowLeft + DebugTheme.rowIndent) {
                    toggleExpanded(node);
                } else {
                    overlay.select(node);
                }
        ...
        protected void renderInternal(SceneCanvas canvas, int mouseX, int mouseY, float partialTicks) {
            int h = height();
            if (isSelected) {
                canvas.fill(0, 0, width(), h, DebugTheme.rowSelectedBg);
                canvas.fill(0, 0, 2, h, DebugTheme.accent);
            } else if (hovered) {
                canvas.fill(0, 0, width(), h, DebugTheme.rowHoverBg);
            }

            // indent guides, aligned with the arrow centers of ancestor rows
            for (int d = 1; d <= depth; d++) {
                int guideX = d * DebugTheme.rowIndent - DebugTheme.rowIndent / 2;
                canvas.fill(guideX, 0, 1, h, DebugTheme.indentGuide);
            }

            var font = Minecraft.getInstance().font;
            int textY = (h - font.lineHeight) / 2 + 1;
            int x = depth * DebugTheme.rowIndent + 1;
            if (hasChildren) {
                var icon = isExpanded ? Textures.arrowDown : Textures.arrowRight;
                int iconY = (h - 10) / 2;
                canvas.color(DebugTheme.textDim).texture(icon, x, iconY, 10, 10).resetColor();
            }
            x += DebugTheme.rowIndent;

            int nameColor = node.visible() ? DebugTheme.text : DebugTheme.textDim;
            ...
            canvas.drawString(size, x + 6, textY, DebugTheme.textDim, false);
        }
    }
}
