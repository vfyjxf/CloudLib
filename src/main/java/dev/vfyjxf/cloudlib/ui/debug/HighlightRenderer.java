...
final class HighlightRenderer {
    ...
    private void drawWidget(GuiGraphics graphics, Widget widget, boolean strong) {
        ...
        if (mL > 0 || mT > 0 || mR > 0 || mB > 0) {
            graphics.fill(x - mL, y - mT, x + w + mR, y + h + mB, DebugTheme.HL_MARGIN);
        }
        if (bL > 0 || bT > 0 || bR > 0 || bB > 0) {
            graphics.fill(x, y, x + w, y + h, DebugTheme.HL_BORDER);
        }
        if (pL > 0 || pT > 0 || pR > 0 || pB > 0) {
            graphics.fill(x + bL, y + bT, x + w - bR, y + h - bB, DebugTheme.HL_PADDING);
        }
        graphics.fill(x + bL + pL, y + bT + pT, x + w - bR - pR, y + h - bB - pB, DebugTheme.HL_CONTENT);

        int outline = strong ? DebugTheme.HL_OUTLINE : DebugTheme.HL_OUTLINE_HOVER;
    ...
    private void drawLabel(GuiGraphics graphics, Widget widget, int x, int y, int w, int h) {
        ...
        graphics.fill(labelX, labelY, labelX + labelW, labelY + labelH, DebugTheme.HL_LABEL_BG);
        graphics.drawString(font, label, labelX + 3, labelY + 2, DebugTheme.HL_LABEL_TEXT, false);
    }
