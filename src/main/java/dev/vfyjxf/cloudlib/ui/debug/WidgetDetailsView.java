...
final class WidgetDetailsView extends WidgetGroup<Widget> {
    ...
    private static final int propPadLeft = 6;
    private static final int propNameWidth = 120;
    private static final int propLineHeight = 12;
    ...
    WidgetDetailsView(DebugOverlayImpl overlay) {
        ...
        useStyle(UIStyle.of(UIStyles.flexColumn(), UIStyles.padding(4, 0, 0, 0), UIStyles.rowGap(2)));
    ...
    private void rebuild() {
        ...
        if (target == null || !target.lifecycle().mounted()) {
            ...
            header.setColor(DebugTheme.textDim);
            return;
        }
    ...
    private static int valueColor(InspectionProperty prop) {
        if (!prop.isNonDefault()) return DebugTheme.propValueDefault;
        String value = prop.value();
        if (value.startsWith("\"")) return DebugTheme.propValueString;
        if (value.equals("true") || value.equals("false")) return DebugTheme.propValueBool;
        if (value.equals("null")) return DebugTheme.propValueDefault;
        if (number.matcher(value).matches()) return DebugTheme.propValueNumber;
    ...
    private static final class SectionHeader extends Widget {
        ...
        protected void renderInternal(SceneCanvas canvas, int mouseX, int mouseY, float partialTicks) {
            ...
            canvas.drawString(title, propPadLeft, textY, DebugTheme.category, false);
            int lineX = propPadLeft + font.width(title) + 6;
            if (lineX < width() - 4) {
                canvas.fill(lineX, height() / 2, width() - 4 - lineX, 1, DebugTheme.sectionLine);
            }
        }
    }
    ...
    private static final class PropRow extends Widget {
        ...
        PropRow(InspectionProperty prop) {
            this.prop = prop;
            useStyle(UIStyle.of(UIStyles.widthOf(TaffyDimension.percent(1)), UIStyles.flexShrink(0)));
            onMount((scene, context, handle) -> scene.layoutTree().setMeasureFunc(nodeId(), (known, available) -> {
                float w = available.width.unwrapOr(240);
                int valueWidth = (int) w - propPadLeft - propNameWidth - 12;
                int lines = wrap(prop.value(), valueWidth).size();
                return new FloatSize(w, lines * propLineHeight + 4);
            }));
        ...
        protected void renderInternal(SceneCanvas canvas, int mouseX, int mouseY, float partialTicks) {
            if (hovered) {
                canvas.fill(0, 0, width(), height(), DebugTheme.rowHoverBg);
            }
            var font = Minecraft.getInstance().font;
            int y = 2;

            String name = prop.name() + ":";
            if (font.width(name) > propNameWidth) {
                while (name.length() > 1 && font.width(name + "…") > propNameWidth) {
                    name = name.substring(0, name.length() - 1);
                }
                name += "…";
            }
            canvas.drawString(name, propPadLeft, y, DebugTheme.propName, false);

            int valueX = propPadLeft + propNameWidth + 8;
    ...
    private final class BoxModelView extends Widget {
        ...
        private static final int height = DebugTheme.bmBand * 6 + DebugTheme.bmContentMin;
        ...
        protected void renderInternal(SceneCanvas canvas, int mouseX, int mouseY, float partialTicks) {
            ...
            int b = DebugTheme.bmBand;
            canvas.fill(0, 0, w, h, DebugTheme.bmMargin);
            canvas.fill(b, b, w - 2 * b, h - 2 * b, DebugTheme.bmBorder);
            canvas.fill(2 * b, 2 * b, w - 4 * b, h - 4 * b, DebugTheme.bmPadding);
            canvas.fill(3 * b, 3 * b, w - 6 * b, h - 6 * b, DebugTheme.bmContent);
        ...
        private int bandCenter(int bandTop) {
            return bandTop + (DebugTheme.bmBand - 9) / 2 + 1;
        }
        ...
        private void drawBandValue(SceneCanvas canvas, Font font, float value, int centerX, int y) {
            ...
            canvas.drawString(text, centerX - font.width(text) / 2, y, DebugTheme.bmText, false);
        }
