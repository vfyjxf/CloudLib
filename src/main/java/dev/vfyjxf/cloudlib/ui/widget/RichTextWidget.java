 ...
 * {@code text-align} style property ({@link Styles#textAlign}), or an explicit
...
public class RichTextWidget extends CompositeWidget<Widget> {
    ...
    private RichTextWidget(RichText text) {
        ...
        style().addChangeListener(Styles.textAlign, (oldValue, newValue) -> onAlignmentChanged());
    ...
    private TextAlignment currentAlignment() {
        ...
        if (styled == null) return TextAlignment.left;
    ...
    private void syncEmbeddedWidgets() {
        ...
        for (WidgetNode node : nodes) {
            ...
            if (widget.parent() != this) {
                widget.useStyle(UIStyle.of(UIStyles.positionAbsolute(), UIStyles.sizeOf(node.width(), node.height())));
    ...
    private void positionEmbedded(WidgetNode node, LayoutScope scope) {
    ...
    private void performClick(ClickAction action, ClickAction.Context context) {
        switch (action) {
            ...
            case ClickAction.CopyToClipboard copy -> Minecraft.getInstance().keyboardHandler.setClipboard(copy.text());
    ...
    private void handleVanillaClick(ClickEvent event) {
        switch (event.getAction()) {
            ...
            case COPY_TO_CLIPBOARD -> Minecraft.getInstance().keyboardHandler.setClipboard(event.getValue());
            case RUN_COMMAND -> {
    ...
    private @Nullable Tooltip vanillaHoverTooltip(HoverEvent event) {
        ...
        if (action == HoverEvent.Action.SHOW_ENTITY) {
            ...
            if (info != null) {
                ...
                tooltip.add(Component.literal(info.type.getDescriptionId()).withStyle(ChatFormatting.GRAY));
                return tooltip;
            }
        }
        return null;
    ...
    protected void renderInternal(SceneCanvas canvas, int mouseX, int mouseY, float partialTicks) {
        ...
        RenderOptions options = RenderOptions.defaults.withDefaultColor(inkColor()).withShadow(shadow)
                .withMouse(mouseX - contentX(), mouseY - contentY()).withPartialTicks(partialTicks)
