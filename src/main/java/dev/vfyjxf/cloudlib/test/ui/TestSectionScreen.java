...
public class TestSectionScreen extends BasicScreen {
    ...
    private void buildUI() {
        ...
        var label1 = LabelWidget.of("这是第一个分组的内容").setColor(0xFFE2E8F0).setAlign(LabelWidget.TextAlign.CENTER);
        ...
        // === 底部提示 ===
        var hint = LabelWidget.of("提示：点击分组标题可以展开/折叠内容").setColor(0xFF94A3B8).setAlign(LabelWidget.TextAlign.CENTER);
    ...
    private ColumnWidget createSection(String title, boolean expanded) {
        ...
        var arrow = LabelWidget.of(expanded ? "▼" : "▶").setColor(0xFFE2E8F0);
        arrow.useStyle(UIStyle.of(sizeOf(12, 12)));
        header.addWidget(arrow);

        var titleLabel = LabelWidget.of(title).setColor(0xFFE2E8F0).setShadow(true);
