...
public final class RichTextLayouter {
     ...
     */
    private static final int TEXT_DESCENT = 2;
     ...
     */
    private record FlattenContext(
            RichTextStyle style,
            @Nullable ClickAction click,
            @Nullable HoverAction hover
    ) {
    ...
    private ObjectAtom objectAtom(RichNode node, int width, int height, FlattenContext context) {
        ...
        return new ObjectAtom(
                ...
                context.style, context.effectiveClick(), context.effectiveHover()
        );
    }
    ...
    private void flattenComponent(Component component, FlattenContext context, RichNode source, List<InlineAtom> out) {
        ComponentAdapter.flatten(component, context.style.style(), (text, style) -> {
            ...
            FlattenContext segmentContext = new FlattenContext(
                    effective, context.click(), context.hover());
    ...
    private void splitText(String text, FlattenContext context, RichNode source, List<InlineAtom> out) {
        ...
        while (i < n) {
            ...
            if (c == '\n') {
                ...
                out.add(new SpaceAtom(
                        spaceWidth * count,
                        context.style, context.effectiveClick(), context.effectiveHover(), source
                ));
            } else {
                word.append(c);
                i++;
    ...
    private LineBuild breakWord(WordAtom word, int maxWidth, List<LineBuild> lines, LineBuild current) {
        ...
        while (!remaining.isEmpty()) {
            ...
            place(current, new WordAtom(
                    part,
                    measurer.width(part, word.style().style()),
                    word.style(), word.onClick(), word.onHover(), word.source()
            ));
    ...
    private TextFragment materialize(PlacedAtom placed, float xStart, float lineY, float lineHeight, int textHeight) {
        ...
        return switch (atom) {
            case WordAtom word -> new TextFragment(
                    ...
                    x, lineY + lineHeight - textHeight, word.width(), textHeight,
                    word.text(), word.style(), word.source(), word.onClick(), word.onHover()
            );
            case SpaceAtom space -> new TextFragment(
                    TextFragment.Kind.SPACE,
                    x, lineY + lineHeight - textHeight, space.width(), textHeight,
                    null, space.style(), space.source(), space.onClick(), space.onHover()
            );
            case ObjectAtom object -> {
                ...
                yield new TextFragment(
                        ...
                        x, lineY + offsetY, object.width(), height,
                        null, object.style(), object.node(), object.onClick(), object.onHover()
                );
            }
