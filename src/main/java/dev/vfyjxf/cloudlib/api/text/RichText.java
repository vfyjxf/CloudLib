...
public final class RichText {
    ...
    private static final RichText none = new RichText(new GroupNode(List.of()));
    ...
    public static final class Builder {
        ...
        /**
         * Points the pen at a theme color slot ({@code --name}), resolved against the
         * active theme while rendering — a theme hot reload repaints without
         * rebuilding the document.
         * <p>
         * A literal color set earlier on the pen stays as the fallback for a theme
         * that does not bind the slot, so
         * {@code color(0xFFAA00).color(Slots.accent)} means "the theme's accent,
         * orange when there is none". See
         * {@link RichTextStyle#textColor(ThemeColorResolver, int)} for the full
         * resolution order.
         */
        @Contract("_ -> this")
        public Builder color(StyleVar<Integer> slot) {
            pen = pen.withColorVar(slot);
            return this;
        }
        ...
        private Builder decorateLast(@Nullable ClickAction onClick, @Nullable HoverAction onHover) {
            if (children.isEmpty()) {
