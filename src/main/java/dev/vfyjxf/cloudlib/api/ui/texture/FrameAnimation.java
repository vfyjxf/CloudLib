 ...
 */
public class FrameAnimation implements Playable<Texture> {

    private record Frame(Texture texture, float duration) {}
    ...
    public FrameAnimation addFrame(Texture texture, float duration) {
        frames.add(new Frame(texture, duration));
        return this;
    }

    public FrameAnimation addFrames(float duration, Texture... textures) {
     ...
     */
    public Texture frameAt(int index) {
        if (index < 0 || index >= frames.size()) return Texture.EMPTY;
    ...
    public Texture value() {
        if (frames.isEmpty()) return Texture.EMPTY;
        return frames.get(currentIndex).texture;
    }

    @Override
    public Texture value(float partialTick) {
        // 帧动画是离散的，直接返回当前帧
