...
public class TextureBatch implements BatchableTexture.BatchCollector {
    ...
    private final List<Consumer<GuiGraphics>> customDraws = new ObjectArrayList<>();
    ...
    public void addQuad(ResourceLocation texture, int x, int y, int width, int height,
        ...
        batches.computeIfAbsent(texture, k -> new ObjectArrayList<>())
     ...
     */
    public TextureBatch draw(Texture texture, int x, int y, int width, int height) {
        checkBuilding();
     ...
     */
    public TextureBatch drawRaw(ResourceLocation texture, int x, int y, int width, int height,
    ...
    public TextureBatch drawSprite(ResourceLocation spriteLocation, int x, int y, int width, int height) {
        ...
        customDraws.add(graphics -> {
            if (color != 0xFFFFFFFF) {
                ...
                RenderSystem.enableBlend();
                RenderSystem.setShaderColor(r, g, b, a);
                graphics.blitSprite(spriteLocation, fx, fy, fw, fh);
                RenderSystem.setShaderColor(1f, 1f, 1f, 1f);
            } else {
                graphics.blitSprite(spriteLocation, fx, fy, fw, fh);
            }
