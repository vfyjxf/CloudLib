...
public final class TextFieldBlueprint implements Blueprint<TextFieldWidget> {
    ...
    public void updateWidget(TextFieldWidget widget, Scene scene, SceneContext context) {
        widget.setText(text).setPlaceholder(placeholder).setMaxLength(maxLength).setEditable(editable)
                .setTextColor(textColor).setPlaceholderColor(placeholderColor).onTextChanged(onTextChanged)
                .onEnterPressed(onEnterPressed).useStyle(style);
    }
