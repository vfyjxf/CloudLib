...
uniform int  Marker;       // 0 = none, 1 = dot (face/area), 2 = arrow (edge/enemy), 3 = bracket (directional)
...
void main() {
    ...
    // edge/enemy one, a corner bracket for a directional target — each
    // appearing once the reveal has reached that end
    float mark = 0.0;
    if (Marker > 0 && MarkerSize > 0.0) {
        ...
        if (Marker == 2) {
            ...
            // a corner bracket: two L arms off the end — one along the
            // arrival direction, one perpendicular — a thin bracket
