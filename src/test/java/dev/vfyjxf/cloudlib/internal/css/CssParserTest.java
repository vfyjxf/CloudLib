...
import dev.vfyjxf.cloudlib.internal.css.ComponentValue.Function;
import dev.vfyjxf.cloudlib.internal.css.ComponentValue.HashValue;
import dev.vfyjxf.cloudlib.internal.css.ComponentValue.Ident;
import dev.vfyjxf.cloudlib.internal.css.ComponentValue.NumericValue;
import dev.vfyjxf.cloudlib.internal.css.ComponentValue.StringValue;
import dev.vfyjxf.cloudlib.internal.css.ComponentValue.UrlValue;
import dev.vfyjxf.cloudlib.internal.css.ComponentValue.Whitespace;
...
class CssParserTest {
    ...
    void customPropertyKeepsRawValue() {
        ...
        HashValue hash = cast(
                HashValue.class,
                d.value().stream()
                        .filter(v -> v != Whitespace.instance)
                        .findFirst()
                        .orElseThrow());
    ...
    void stringsAndUrlsAndEscapes() {
        StyleRule rule =
                onlyRule(sheet("a { content: \"it\\'s\"; font: 'x'; background: url(img.png); alt: url(\"q.png\") }"));
        var decls = rule.declarations();
    ...
    void edgeShorthandKeepsFourValues() {
        ...
        List<ComponentValue> v = rule.declarations().get(0).value().stream()
                .filter(c -> c != Whitespace.instance)
                .toList();
    ...
    void idClassTagCompound() {
        CompoundSelector s =
                onlySelector(onlyRule(sheet("button.primary#go { x: y }"))).last();
    ...
    void attributeOperators() {
        Object[][] cases = {
            {"=", AttributeSelector.Operator.exact},
            {"~=", AttributeSelector.Operator.includes},
            {"|=", AttributeSelector.Operator.dashMatch},
            {"^=", AttributeSelector.Operator.prefixMatch},
            {"$=", AttributeSelector.Operator.suffixMatch},
            {"*=", AttributeSelector.Operator.substringMatch},
        };
        for (Object[] c : cases) {
            CompoundSelector s = onlySelector(onlyRule(sheet("a[k" + c[0] + "\"v\"] { x: y }")))
                    .last();
    ...
    void attributeFlags() {
        CompoundSelector i =
                onlySelector(onlyRule(sheet("a[k=\"v\" i] { x: y }"))).last();
        assertEquals(
                AttributeSelector.MatchFlag.insensitive, i.attributes().get(0).flag());
        CompoundSelector s =
                onlySelector(onlyRule(sheet("a[k=\"v\" s] { x: y }"))).last();
        assertEquals(
                AttributeSelector.MatchFlag.sensitive, s.attributes().get(0).flag());
    }
    ...
    void pseudoElementIsElement() {
        CompoundSelector s =
                onlySelector(onlyRule(sheet("a::part(x) { x: y }"))).last();
        PseudoClass p = s.pseudos().get(0);
    ...
    void notTakesMaxArgSpecificity() {
        CompoundSelector s =
                onlySelector(onlyRule(sheet("a:not(#x.b) { x: y }"))).last();
        PseudoClass p = s.pseudos().get(0);
        PseudoArgs.SelectorList args = cast(PseudoArgs.SelectorList.class, p.args());
        assertEquals(
                new Specificity(1, 1, 0), args.selectors().get(0).selector().specificity());
        // a + max(#x.b = 1,1,0)
        assertEquals(new Specificity(1, 1, 1), s.specificity());
    }
    ...
    void whereAddsNoSpecificity() {
        CompoundSelector s =
                onlySelector(onlyRule(sheet("a:where(#x) { x: y }"))).last();
    ...
    void hasAllowsLeadingCombinator() {
        CompoundSelector s =
                onlySelector(onlyRule(sheet("a:has(> b) { x: y }"))).last();
        PseudoArgs.SelectorList args =
                cast(PseudoArgs.SelectorList.class, s.pseudos().get(0).args());
    ...
    void isWithSelectorList() {
        CompoundSelector s =
                onlySelector(onlyRule(sheet("a:is(.x, #y) { x: y }"))).last();
        PseudoArgs.SelectorList args =
                cast(PseudoArgs.SelectorList.class, s.pseudos().get(0).args());
    ...
    @Test
    void nthOddEven() {
        PseudoClass odd = onlySelector(onlyRule(sheet("a:nth-child(odd) { x: y }")))
                .last()
                .pseudos()
                .get(0);
        ...
        assertEquals(2, ab.a());
        assertEquals(1, ab.b());
        PseudoClass even = onlySelector(onlyRule(sheet("a:nth-child(even) { x: y }")))
                .last()
                .pseudos()
                .get(0);
    ...
    void nthForms() {
        Object[][] cases = {
            {"2n+1", 2, 1}, {"-n+3", -1, 3}, {"3n", 3, 0}, {"n", 1, 0},
            {"-n", -1, 0}, {"-2n-1", -2, -1}, {"4", 0, 4}, {"-5", 0, -5},
            {"+2n", 2, 0}, {"3n - 1", 3, -1}, {"n+ 2", 1, 2}, {"-n- 3", -1, -3},
        };
        for (Object[] c : cases) {
            PseudoArgs.AnPlusB ab =
                    (PseudoArgs.AnPlusB) onlySelector(onlyRule(sheet("a:nth-child(" + c[0] + ") { x: y }")))
                            .last()
                            .pseudos()
                            .get(0)
                            .args();
    ...
    void langTakesIdentList() {
        PseudoArgs.Idents args = (PseudoArgs.Idents) onlySelector(onlyRule(sheet("a:lang(en, fr) { x: y }")))
                .last()
                .pseudos()
                .get(0)
                .args();
    ...
    void unknownFunctionalPseudoKeepsRawArgs() {
        PseudoArgs.Raw args = (PseudoArgs.Raw) onlySelector(onlyRule(sheet("a:mypseudo(1px solid) { x: y }")))
                .last()
                .pseudos()
                .get(0)
                .args();
    ...
    void missingCloseBraceStillParses() {
        ...
        assertEquals(1, s.rules().size());
        assertEquals(
                "color", ((StyleRule) s.rules().get(0)).declarations().get(0).property());
    }
    ...
    void garbageBetweenRulesIsTolerated() {
        ...
        assertEquals(1, s.rules().size());
        assertEquals(
                "a", ((StyleRule) s.rules().get(0)).selectors().get(0).last().tag());
    }
    ...
    void invalidSelectorDropsRule() {
        ...
        assertEquals(1, s.rules().size());
        assertEquals(
                "b", ((StyleRule) s.rules().get(0)).selectors().get(0).last().tag());
    }
